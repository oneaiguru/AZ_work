...
4. Проверьте TLS для Nginx. Контейнер автоматически сгенерирует самоподписанный сертификат в `ops/nginx/certs` (используются переменные `NGINX_SSL_DOMAIN` и `NGINX_SSL_DAYS`). Для боевого домена замените эти файлы собственным `fullchain.pem`/`privkey.pem` (например, полученными через Certbot) и перезапустите прокси: `docker compose up -d nginx`.
5. При необходимости добавьте в `.env` статический токен администратора, чтобы сидер и CI могли авторизоваться без ввода пароля:
   1. Запустите Directus и войдите под супер-админом (`DIRECTUS_ADMIN_EMAIL` / `DIRECTUS_ADMIN_PASSWORD`).
   2. Откройте меню аватара → **Настройки профиля** → вкладка **Токены** → **Создать токен**.
   3. Скопируйте значение и сохраните его в `.env` в переменной `DIRECTUS_ACCESS_TOKEN`.
   4. Перезапустите `docker compose up -d`, чтобы контейнеры перечитали переменные окружения.
   Подробности и рекомендации по хранению токена см. в [docs/directus-content-management.md](docs/directus-content-management.md#как-получить-статический-токен-администратора-directus).
...
> 🔐 При отсутствии боевого сертификата Nginx сгенерирует самоподписанную пару в `ops/nginx/certs`. Браузер предупредит о недоверенном сертификате — примите исключение или установите свой `fullchain.pem`/`privkey.pem` и перезапустите `docker compose up -d nginx`.
