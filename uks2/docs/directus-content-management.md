...
1. Откройте браузер и перейдите по адресу, указанному в переменной `DIRECTUS_PUBLIC_URL` (в боевой конфигурации `https://cms.uks.delightsoft.ru/admin`, локально — `https://cms.uks2.localhost/admin`).
...
1. После сохранения откройте сайт (`https://uks.delightsoft.ru` в продакшне или `https://uks2.localhost` локально) и убедитесь, что изменения отображаются корректно.
