...
3. Дополнительные параметры скрипта:
   ...
   Если Directus блокирует чтение системной коллекции ролей (ошибка `403` на `/roles/public`), задайте переменную `DIRECTUS_PUBLIC_ROLE_ID` с идентификатором публичной роли — сидер использует её для настройки прав без дополнительных запросов.
