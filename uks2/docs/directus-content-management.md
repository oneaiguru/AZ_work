...
3. Дополнительные параметры и переменные скрипта:
   ...
   - `DIRECTUS_ACCESS_TOKEN` — статический токен администратора. Если он задан, скрипт не запрашивает `DIRECTUS_ADMIN_EMAIL`/`DIRECTUS_ADMIN_PASSWORD` и сразу использует токен при авторизации.
     Если API блокирует логин по email/паролю, создайте токен и добавьте его в `.env` (см. инструкцию ниже).
   ...
   Сидер самостоятельно подбирает или создаёт публичную роль и назначает ей нужную политику. Если Directus полностью блокирует чтение системной коллекции ролей (ошибка `403` на всех запросах к `/roles`), задайте переменную `DIRECTUS_PUBLIC_ROLE_ID` с идентификатором публичной роли — скрипт использует её вместо автоматического поиска. В стандартной установке Directus ID публичной роли — `00000000-0000-0000-0000-000000000001`; это значение уже прописано в `.env.example`. Оно не связано с админским доступом: вход в интерфейс осуществляется учёткой `DIRECTUS_ADMIN_EMAIL` / `DIRECTUS_ADMIN_PASSWORD`.

#### Как получить статический токен администратора Directus

1. Запустите Directus и войдите под супер-администратором (`DIRECTUS_ADMIN_EMAIL` / `DIRECTUS_ADMIN_PASSWORD`).
2. Нажмите на аватар в правом верхнем углу → **Настройки профиля**.
3. Откройте вкладку **Токены** и нажмите **Создать токен**.
4. Укажите название (например, `seed-script`), при необходимости срок действия и сохраните.
5. Скопируйте сгенерированное значение и добавьте его в `uks2/.env`:
   ```ini
   DIRECTUS_ACCESS_TOKEN=скопированное-значение
   ```
6. Перезапустите `docker compose up -d`, чтобы контейнеры перечитали переменные окружения, и повторите запуск сидера.

> ⚠️ Статический токен даёт полный доступ от имени пользователя, для которого он создан. Храните значение в секрете и регенерируйте токен при подозрении на компрометацию.
4. Скрипт пытается скачать баннеры и фотографии с `https://uks.irkutsk.ru` (fallback-список лежит в `manualImages`). Если сайт отдаёт `403` на прямые запросы, сидер повторно скачает изображение собственным HTTP-клиентом с заголовком `Referer: https://uks.irkutsk.ru/` и загрузит файл через multipart в Directus. При сетевых ограничениях по-прежнему можно добавить флаг `--skip-images`.
5. После аутентификации сидер проверяет, что учётная запись обладает `admin_access`. Если Directus отвечает `403` на `/users/me` или коллекции контента, убедитесь, что используете супер-админа или статический токен с полными правами, а также применена схема (`npx directus schema apply`).
...
| `seed-directus.js` завершился с `403` на `/roles`/`/access` | Защищены ли системные коллекции от чтения | Включите `DIRECTUS_PUBLIC_ROLE_ID` с актуальным идентификатором публичной роли или временно снимите запрет на чтение системных коллекций, чтобы сидер смог создать/обновить роль |
