...
2. Поля:
   ...
   - `url` — ссылка на файл. Лучше копировать URL из медиатеки (`https://cms…/assets/<id>`), тогда файл остаётся под контролем Directus;
...
3. Дополнительные параметры скрипта:
   ...
   - `--directus-url <URL>` — явно указать базовый адрес Directus, если скрипт запускается вне Docker-сети (например, `http://localhost/cms`);
   - `--token <value>` — использовать заранее созданный статический токен (альтернатива `DIRECTUS_ADMIN_STATIC_TOKEN`);
   - `--token-file <path>` — считать токен из файла (удобно для CI/CD или секретов в хранилище).
4. Скрипт пытается скачать баннеры и фотографии с `https://uks.irkutsk.ru` (fallback-список лежит в `manualImages`). Если домен недоступен, в медиатеке появятся только резервные изображения.
